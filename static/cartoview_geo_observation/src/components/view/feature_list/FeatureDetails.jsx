...
class FeatureDetails extends React.Component {
    ...
    addComment = (data) => {
        const { urls, config, username } = this.props
    ...
    getFeature = () => {
        ...
        const requestUrl = FeatureListHelper.getFeaturesURL(urls.wfsURL,
            0, config.layer, fid, parseInt(config.pagination), map.getView().getProjection().getCode())
                ...
                {!loading && feature && <Paper className={classes.tabelPaper} elevation={0}>
                    <ExpansionPanel defaultExpanded>
                        <ExpansionPanelSummary classes={{ content: "text-wrap" }} expandIcon={<ExpandMoreIcon />}>
                            <Typography className="text-wrap" variant="title" noWrap>{`${feature.getProperties()[config.titleAttribute]}`}</Typography>
                        </ExpansionPanelSummary>
                        <ExpansionPanelDetails classes={{ root: classes.panelDetails }}>
                            {config.attributesToDisplay.map((key, index) => {
