...
class CartoviewPopup extends React.Component {
    ...
    ensureEvents = () => {
        ...
        var detailsB = self.detailsButton
        ...
        if (detailsB.onclick === null) {
            detailsB.onclick = () => {
                let { currentFeature } = this.state
                window.location.href = urls.viewURL + currentFeature.getProperties()._layerName + "/" + currentFeature.getId() + "/details"
            }
        }
    }
                ...
                <Paper elevation={2}>
                    ...
                    <div className="cartoview-popup-actions center">
                        <div ref={(input) => { this.zoomToButton = input }} >
                            {(featureIdentifyResult.length != 0 && !featureIdentifyLoading) && <Button color="default" className={classes.button} size="small">
                                <ZoomIcon />
                                <Typography type="caption" align="left" noWrap={false} color="inherit">{`Zoom`}</Typography>
                            </Button>}
                        </div>
                        <div ref={(input) => { this.detailsButton = input }} >
                            {(featureIdentifyResult.length != 0 && !featureIdentifyLoading) && <Button color="primary" className={classes.button} size="small">
                                <InfoIcon />
                                <Typography type="caption" align="left" noWrap={false} color="inherit">{`Details`}</Typography>
                            </Button>}
                        </div>
                    </div>
                </Paper>
